...
def seq_train(loader, model, optimizer, device, loss_weights_original,epoch_idx, recoder):
    ...
    if epoch_idx < 10:
        loss_weights.pop('DistTranslation2SLR')
        loss_weights.pop('DistTranslation2SLRFeature')
    else:
        loss_weights.pop('TranslationToGlossCrossEntropy')
    ...
    for batch_idx, data in enumerate(loader):
        ...
        if epoch_idx < 10:
            ...
            loss,loss_ctc_RNN,loss_ctc_conv1d,loss_dist_RNN_to_conv1d,loss_dist_fusion_to_slr,loss_dist_translation_to_slr,loss_sign_translation = model.criterion_calculation(0, ret_dict_teacher_translation, ret_dict_student,label, label_lgt, label_text_target,epoch_idx,loss_weights)
        else:
