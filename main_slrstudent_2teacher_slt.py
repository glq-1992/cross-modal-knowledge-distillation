...
faulthandler.enable()
import utils
from seq_2teacher_slt_csldaily import seq_train, seq_eval, seq_feature_generation
...
class Processor():
    def __init__(self, arg):
        ...
        # new label
        ch_vocab_filename = "DatasetFile/dialogue_csldaily/gloss_map.txt"
        with open(ch_vocab_filename, encoding='utf-8') as f:
            vocab = f.readlines()
            self.gloss_dict = {vocab[i].replace('\n', ''):i+1 for i in range(len(vocab))}

        # new label
        ch_vocab_text_filename = "DatasetFile/dialogue_csldaily/char_map.txt"
...
if __name__ == '__main__':
    ...
    sparser = utils.parameters_slrstudent_2teacher_slt.get_parser()
